<?php
  ...
  $count=0;
  $classid1=$_SESSION["classid"];
  $r11=mysql_query("select * from semregstatus where next_sem='$classid1'");   

              while($dat11=mysql_fetch_array($r11))
              {
               $classid=$dat11["curr_sem"];
             }
...
//reading student details
            ...
            while($dat=mysql_fetch_array($resul))
            {
             ...
             $reW=mysql_query("select * from current_class_semreg where studid = '$adm_no' ");
...
<div id="menu3" class="tab-pane fade">
 ...
 <div class="table-responsive">
  ...
  $resul=mysql_query("select studid,name,mobile_phno from current_class_semreg,stud_details where admissionno=studid and classid='$classid' and studid not in(select adm_no from stud_sem_registration where classid='$classid'  ) order by name") or die(mysql_error());
  $serial_no = 0;
  while($dat=mysql_fetch_array($resul))
  { 
    $serial_no++;
