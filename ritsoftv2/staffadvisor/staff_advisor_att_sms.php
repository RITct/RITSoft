...
<div id="page-wrapper">
<h3> Send Attentance Staement to Parents - SMS</h3> <br><br>
	<form  id="form1" name="form1" method="post" enctype="multipart/form-data"> 
		...
		<select name="class" id="class" onChange="showsub(this.value)"  class="form-control">
			...
			while($rs=mysqli_fetch_array($res))
			{
				...
				<br/>
				<input type="submit" onclick="return confirm('Are you sure, Do you want to send the attendance statement-SMS ?');"  name="btnshow" value="submit"  class="btn btn-primary"/><br/>
			</form>
...
<?php
			...
			{
                              $st1="Message sending failed to the parents of following students <br>";

                              $msg="Attendance Stmt of";     
                               $cid11=$_SESSION['cid']; $pp=0;
                               If($cid11=="BTECH" || $cid11=="MTECH") {$pp=1;}
					...
					<tr>
						...
						while($rs=mysqli_fetch_array($res))
						{
							...
							<th><?php 
                                                               ...
                                                               echo $rs["subjectid"]." - ".$rs["subject_title"]; ?></th>
							<?php
						}
						?>
						<th>Total</th>
	          ...
	          $res2=mysqli_query($con3,"SELECT c.studid,b.name,c.rollno FROM stud_details b,current_class c where c.classid='$class[0]' and c.studid=b.admissionno and c.rollno='$j' order by c.rollno asc");
						...
						{  
							...
							<tr>
								...
								while($rs3=mysqli_fetch_array($res3)) {
									...
									if(mysqli_num_rows($res4)==0){
										...
										echo $tbo; 
                                                                                if($tbo == "0%"){
                                                                                $msg=$msg.$subj." ".$subtitle." - ".$tbo."- Attendance shortage,\r\n\r\n";}

									}else {
									echo round(((mysqli_num_rows($res5)/mysqli_num_rows($res4))*100),2)."%";
                                                                           $per11=round(((mysqli_num_rows($res5)/mysqli_num_rows($res4))*100),2);
                                                                       if($pp=1 && $per11<=75.00)

                                                                           {$msg=$msg.$subj." ".$subtitle." - ".$per11."% - Attendane Shortage,\r\n\r\n "; }
                                                                         elseif($pp=0 && $per11<=85.00)
                                                                            {$msg=$msg.$subj." ".$subtitle." - ".$per11."% - Attendane Shortage,\r\n\r\n"; }
                                                                         else { $msg=$msg.$subj." ".$subtitle." - ".$per11."%,\r\n\r\n "; }
							...
							<?php
                                               ...
                                               $admno11=$_SESSION['admis'];

                                              include_once('att_sms_parent.php');
                                              $y=sendattsms($msg,$admno11);
                                              if(!$y){ $st1=$st1.$admno11." ".$name."<br>"; }
...
echo "<script>window.location.href='smsstatus.php?status=".$st1."'</script>";
