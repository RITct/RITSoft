...
if(isset($_POST['excel']))
{
  ...
  $sql = "SELECT * FROM stud_details WHERE ($category='$add_no' AND status like 'On Going') AND (courseid LIKE 'btech' OR courseid LIKE 'barch')";
...
// rename the sheet
  $objSheet->setTitle(' UG STUDENT DETAIL');
...
// as you can see, we can specify a range of cells, like here: cells from A1 to A4
  ...
  $objSheet->getCell('L1')->setValue('roll_no');
  ...
  $objSheet->getCell('W1')->setValue('no_chance');
  ...
  while($row = mysqli_fetch_array($result)){
   ...
   $objSheet->getCell('U'.$i)->setValue($row['dob']);
   $objSheet->getCell('V'.$i)->setValue($row['percentage_2']);
