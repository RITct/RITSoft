...
function getdept(course) {		
	...
	function getsemester(course,department,specialization) {		
		...
		showDate ( course, department, specialization) ;
	}




	function showDate ( course, dept, spec) {
		...
		if (req) {
			...
			req.open("GET","getDate.php?course="+course+"&dept="+dept+"&spec="+spec,true);
			req.send();
			...
			req.onreadystatechange=function() 
			{
				...
				{
					document.getElementById("pdate").innerHTML=req.responseText; 
...
<div id="page-wrapper">
	<form id="form1" name="form1" method="post" action="">
		...
		<div id="specialization">
			<select name="spec" id="spec" onChange="doChangesg(this.value)"  class="form-control">
				<option>select </option>
			</select>
		</div>
	...
	//$res2=mysqli_query($con,"SELECT * FROM current_class c,stud_sem_registration s where c.classid='$classid' and s.new_sem='S$semester' and c.studid=s.adm_no order by rollno asc");
		...
		//$percent=0;
						...
						?>
						<td><?php  echo round(($percent*100),2); ?></td>
						<?php
					}
					?>    
					<td><?php 
				...
				<a target="_blank" href="ugdean_xl.php?cls=<?php echo $classid; ?>&e=<?php echo $date1; ?>&f=<?php echo $date2; ?>">Export to Excel</a>
