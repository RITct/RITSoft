...
if(isset($_POST['add_no']))
{
	$add_no=$_POST['add_no'];
	$_SESSION['adm']=$add_no;
	$sql = "SELECT stud_details.* FROM stud_details,current_class WHERE admissionno=studid and (admissionno LIKE '" . $add_no . "%' AND status like 'On Going')";
	if($result = mysqli_query($con, $sql)){
		if(mysqli_num_rows($result) > 0){
			...
			<th style='text-align:center'>NO_CHANCE</th>-->
...
if(isset($_POST['name']))
{
	...
	$sql = "SELECT stud_details.* FROM stud_details,current_class WHERE admissionno=studid and (name LIKE '" . $name . "%' AND status like 'On Going')";
	if($result = mysqli_query($con, $sql)){
		if(mysqli_num_rows($result) > 0){
