...
if($cid=='MCA'|| $cid =='BARCH')
{
	...
	$html.='<h2 style=margin-top:0 ><center>'.$na . ' <br/> ' .$semester.' '.$cid.' duty leave </center></h2>';
}
else
{
	
	$html.='<h2 style=margin-top:0 ><center> '.$na . '  <br/> '.$cid.' '.ucwords($bs).'   Duty Leave </center></h2>';
}
...
$res2=mysqli_query($con3,"SELECT *,  DATE_FORMAT(date, '%D %M,%Y %r' ) AS date  FROM `duty_leave` d LEFT JOIN stud_details s ON s.admissionno = d.studid LEFT JOIN current_class c ON c.studid = d.studid  where c.classid='$cls'  and d.subjectid='$j' order by c.rollno asc");
