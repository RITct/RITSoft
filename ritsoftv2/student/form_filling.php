...
if( !isset($_POST['admission_no']) || ! isset($_POST['class_id']) ) {
	echo "<script>window.location.href='semregpost.php'</script>";
	exit();
}
...
if(mysql_num_rows($l) > 0)
	if( !isset($_POST['Re-Submit']) &&  !isset($_POST['Re-print'])) 
		echo "<script>window.location.href='semregview.php'</script>";
	else{
			...
			<?php if ($detect->isMobile() ) :?>
				...
				<div id="page-wrapper">
					...
					<div class="text-center" style="margin: 3rem ;">
						<form id="myForm" action="sem_reg_from_mobile.php"  class="fomr "   method="post">
							<?php
							foreach ($json as $a => $b) {
							...
							<div class="form-group ">
								<div class="button-group">
									<a class="btn btn-lg btn-warning" style="margin-right: 1rem;" href="semregview.php" type="subject">view status</a>
				...
				<?php else: ?> 
					...
					<form id="myForm" action="sem_reg_from.php"    method="post">
						<?php
...
$l=mysql_query("select classid from current_class where studid='$admissionno'") or die(mysql_error());
if(mysql_num_rows($l)>0)
{
...
$result=mysql_query("select semid from class_details where classid =(select classid from current_class where studid = '$admissionno' )");
...
$re=mysql_query("select * from current_class where studid = '$admissionno' ");
...
$re=mysql_query(" SELECT sd.* , cc.rollno AS rollnoOR FROM stud_details sd LEFT JOIN current_class cc ON cc.studid = sd.admissionno WHERE sd.admissionno =  '$admissionno' ");
$name = '';
$year_of_admission = '';
$branch = '';
...
<div id="page-wrapper">
	...
	<!-- registration -->
	<form id="form1" name="form1" method="post" action="registration.php" enctype="multipart/form-data" class="sear_frm" >
