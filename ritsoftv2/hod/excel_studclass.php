<?php
    if(isset($_POST['excel']))
    {
        ...
        $sql = "SELECT * FROM stud_details A LEFT JOIN current_class B ON A.admissionno=B.studid LEFT JOIN class_details C ON B.classid=C.classid WHERE A.courseid='$search_query[0]' AND C.semid='$search_query[1]' AND C.deptname='$search_query[3]' AND C.branch_or_specialisation='$search_query[2]' ";
...
$objSheet->getCell('N1')->setValue('rank');
$objSheet->getCell('O1')->setValue('quota');
$objSheet->getCell('P1')->setValue('school_1');
$objSheet->getCell('Q1')->setValue('regno_1');
$objSheet->getCell('R1')->setValue('board_1');
$objSheet->getCell('S1')->setValue('percentage_1');
$objSheet->getCell('T1')->setValue('school_2');
$objSheet->getCell('U1')->setValue('regno_2');
$objSheet->getCell('V1')->setValue('board_2');
$objSheet->getCell('W1')->setValue('percentage_2');
...
$objSheet->getCell('Y1')->setValue('branch_or_specialisation');
     ...
     $i=2;
       ...
       $objSheet->getCell('N'.$i)->setValue($row['rank']);
       $objSheet->getCell('O'.$i)->setValue($row['quota']);
       $objSheet->getCell('P'.$i)->setValue($row['school_1']);
       $objSheet->getCell('Q'.$i)->setValue($row['regno_1']);
       $objSheet->getCell('R'.$i)->setValue($row['board_1']);
       $objSheet->getCell('S'.$i)->setValue($row['percentage_1']);
       $objSheet->getCell('T'.$i)->setValue($row['school_2']);
       $objSheet->getCell('U'.$i)->setValue($row['regno_2']);
       $objSheet->getCell('V'.$i)->setValue($row['board_2']);
       $objSheet->getCell('W'.$i)->setValue($row['percentage_2']);
