...
if(isset($_POST['add_no']))
{
    $add_no=$_POST['add_no'];
    $_SESSION['adm']=$add_no;
    $sql = "SELECT * FROM stud_details WHERE (admissionno='$add_no' AND status like 'On Going' AND (courseid LIKE 'mtech' OR courseid LIKE 'mca'))";
    if($result = mysqli_query($con, $sql)){
        if(mysqli_num_rows($result) > 0){
            ...
            echo "</table>"; 
            echo "</div>"; 
            echo "<form method='post' action='sessionmark.php'>";
...
if(isset($_POST['name']))
{
 ...
 $sql = "SELECT * FROM stud_details WHERE (name='$name' AND status like 'On Going' AND (courseid LIKE 'mtech' OR courseid LIKE 'mca'))";
 if($result = mysqli_query($con, $sql)){
    if(mysqli_num_rows($result) > 0){
        ...
        while($row = mysqli_fetch_array($result)){
            ...
            // echo '<td><a href="sessionmark.php?id='.$row['admissionno'].'">VIEW</a></td>';
            echo "</tr>";
        }
        echo "</table>";
        echo "</div>";
